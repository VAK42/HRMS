      ...
      <main className="pt-24 px-4 lg:px-8 pb-8">
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div><h1 className={`text-2xl font-bold ${isDark ? "text-white" : "text-black"}`}>Cấu Hình</h1><p className={`mt-1 ${isDark ? "text-white" : "text-black"}`}>Cài Đặt Hệ Thống</p></div>
            <Button variant="dark" onClick={handleSave} loading={saving}><Save className="w-4 h-4" /> Lưu Thay Đổi</Button>
          </div>
          <Card>
            <CardHeader><CardTitle>Thiết Lập Chung</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              {loading ? (
                ...
                <p className={`text-center py-8 ${isDark ? "text-white/50" : "text-black/50"}`}>Không Có Cấu Hình</p>
              ) : (
                configs.map(config => (
                  <Input
                    key={config.id}
