...
export default function DocumentsPage() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedFile) return alert("Vui lòng chọn tập tin")
    const payload = {
  ...
  const handleDelete = async (id: number) => { if (confirm("Xóa Tài Liệu Này?")) { await api.delete(`/documents/${id}`); fetchData() } }
  return (
    <><Navbar /><main className="pt-24 px-4 lg:px-8 pb-8"><div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className={`text-2xl font-bold ${isDark ? "text-white" : "text-black"}`}>Tài Liệu</h1><p className={`mt-1 ${isDark ? "text-white" : "text-black"}`}>Kho Lưu Trữ</p></div>
        <Button variant="dark" onClick={() => { setFormData({ employeeId: "", documentType: "", fileName: "" }); setSelectedFile(null); setModalOpen(true) }}><Upload className="w-4 h-4" /> Tải Lên Tài Liệu</Button>
      </div>
      <Card><CardContent className="p-6">
        {loading ? (
          <div className="space-y-4">{[...Array(5)].map((_, i) => <div key={i} className="h-16 bg-white/5 rounded-xl animate-pulse" />)}</div>
        ) : documents.length === 0 ? (
          <div className="text-center py-12"><FileText className={`w-12 h-12 mx-auto mb-4 ${isDark ? "text-white/50" : "text-black/50"}`} /><p className={isDark ? "text-white/50" : "text-black/50"}>Chưa Có Tài Liệu Nào</p></div>
        ) : (
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Tải Lên Tài Liệu" size="md">
        <form onSubmit={handleUpload} className="space-y-4">
          ...
          <Select label="Loại Tài Liệu" value={formData.documentType} onChange={(e) => setFormData({ ...formData, documentType: e.target.value })} options={[{ value: "", label: "Chọn Loại" }, { value: "ID Card", label: "CMND/CCCD" }, { value: "Passport", label: "Hộ Chiếu" }, { value: "Contract", label: "Hợp Đồng" }, { value: "Certificate", label: "Chứng Chỉ" }, { value: "Resume", label: "Sơ Yếu Lý Lịch" }, { value: "Other", label: "Khác" }]} required />
          <div>
            <label className={`block text-sm font-medium mb-2 ${isDark ? "text-white" : "text-black"}`}>Chọn Tập Tin</label>
            ...
            {selectedFile && <p className={`mt-2 text-sm ${isDark ? "text-white/70" : "text-black/70"}`}>Đã Chọn: {selectedFile.name} ({(selectedFile.size / 1024).toFixed(1)} KB)</p>}
          </div>
          <Input label="Tên Tập Tin" value={formData.fileName} onChange={(e) => setFormData({ ...formData, fileName: e.target.value })} required />
          <div className="flex justify-end gap-2 pt-4"><Button type="button" variant="secondary" onClick={() => setModalOpen(false)}>Hủy</Button><Button type="submit">Tải Lên</Button></div>
        </form>
      </Modal></>
  )
}